...
import {
  ...
  FileScan, FileText, Gavel, GitCompareArrows, LayoutDashboard, LogOut, Menu, Package, PackageCheck,
  Radar, Search, Settings, ShieldCheck, Users, X
} from "lucide-react";
...
const navGroups: NavGroup[] = [
  ...
  },
  {
    label: "Catálogo",
    items: [
      { href: "/produtos", icon: Package, label: "Central de Produtos" },
      { href: "/equivalencias", icon: GitCompareArrows, label: "Compêndio de Equivalências" },
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const sidebar = (
    <aside className="flex h-full w-[248px] shrink-0 flex-col border-r border-slate-200 bg-white text-slate-900">
      <div className="relative border-b border-slate-200 px-5 py-5">
        ...
        <button className="absolute right-3 top-3 rounded-md p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700 lg:hidden" onClick={() => setSidebarOpen(false)} aria-label="Fechar menu"><X size={17} /></button>
      </div>
          ...
          return (
            <section key={group.label} className="mb-2">
              <button type="button" className="flex w-full items-center justify-between rounded-lg px-2 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-slate-500 hover:bg-slate-50" onClick={() => setCollapsed((state) => ({ ...state, [group.label]: !state[group.label] }))} aria-expanded={!isCollapsed}>
                {group.label}<ChevronDown size={13} className={`transition-transform ${isCollapsed ? "-rotate-90" : ""}`} />
              </button>
              {!isCollapsed && (
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-2.5 rounded-lg px-3 py-2 text-[13px] no-underline transition-colors ${active ? "bg-blue-50 font-bold text-blue-950" : "font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-950"}`}>
                        <Icon size={16} strokeWidth={active ? 2.2 : 1.7} className="shrink-0" /><span className="truncate">{item.label}</span>
                      </Link>
                    );
                  })}
                </div>
      ...
      <div className="border-t border-slate-200 p-3">
        {isAuthenticated ? (
          <div className="rounded-xl bg-slate-50 p-3">
            ...
            <div className="mt-1 flex items-center justify-between"><span className="text-[10px] text-slate-500">{getRoleLabel(user?.role)}</span><button onClick={() => logout()} className="flex items-center gap-1 rounded-md px-2 py-1 text-[10px] font-bold text-slate-500 hover:bg-white hover:text-red-700"><LogOut size={12} /> Sair</button></div>
          </div>
        ) : (
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50">
      ...
      {sidebarOpen && <div className="fixed inset-0 z-50 lg:hidden"><button className="absolute inset-0 h-full w-full bg-slate-950/45" onClick={() => setSidebarOpen(false)} aria-label="Fechar menu" /><div className="absolute inset-y-0 left-0">{sidebar}</div></div>}
      <div className="flex h-full min-w-0 flex-1 flex-col">
        {isAuthenticated && <header className="flex h-14 shrink-0 items-center gap-3 border-b border-slate-200 bg-white px-4 sm:px-6"><button className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 lg:hidden" onClick={() => setSidebarOpen(true)} aria-label="Abrir menu"><Menu size={19} /></button><div className="min-w-0"><p className="m-0 truncate text-sm font-extrabold text-slate-900">{currentPageLabel(location)}</p></div><Link href="/busca-global" className="ml-auto flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-medium text-slate-500 no-underline hover:border-slate-300 hover:bg-white"><Search size={14} /><span className="hidden sm:inline">Buscar no sistema</span></Link></header>}

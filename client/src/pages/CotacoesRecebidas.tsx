...
function Workspace({ data, canEdit, smtp, onBack, onChanged }: { data: any; canEdit: boolean; smtp: boolean; onBack: () => void; onChanged: () => void }) {
  ...
  return <div>
    ...
    <div className="space-y-3 p-4">{visible.map((item) => <ItemCard key={item.id} item={item} d={decisionMap.get(item.id)} p={previewMap.get(item.id)} canEdit={canEdit} draft={sale[item.id]} setDraft={(v) => setSale((s) => ({ ...s, [item.id]: v }))} saveSale={() => saveSale(item.id)} approve={() => item.produtoMatchId && useProduct(item, { id: item.produtoMatchId, price: item.precoSugerido ?? item.productPrice })} useMemory={(m) => useProduct(item, { id: m.productId, price: m.lastCost == null ? undefined : String(m.lastCost) })} useSupplier={(s) => item.produtoMatchId && useProduct(item, { id: item.produtoMatchId, price: s.effectivePrice == null ? undefined : String(s.effectivePrice) })} onPick={() => setPicker(item)} onCompare={() => setCompare(item)} onCreate={() => setCreate(item)} />)}</div>

    {picker && <ProductPicker item={picker} onClose={() => setPicker(null)} onCreate={() => { setCreate(picker); setPicker(null); }} onUse={(product) => useProduct(picker, product)} />}
    {compare && <Compare item={compare} d={decisionMap.get(compare.id)} onClose={() => setCompare(null)} onUse={(product) => useProduct(compare, product)} />}
    {create && <QuickCreate item={create} onClose={() => setCreate(null)} onCreated={(product) => { useProduct(create, product); setCreate(null); }} />}
  </div>;
}

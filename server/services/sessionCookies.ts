...
function validCookie(cookie: unknown): cookie is PuppeteerCookieLike {
  ...
  return typeof value.name === "string" && value.name.length > 0 && typeof value.value === "string";
}

function compactCookie(cookie: PuppeteerCookieLike): PuppeteerCookieLike {
  const result: PuppeteerCookieLike = { name: cookie.name, value: cookie.value };
  if (cookie.domain) result.domain = cookie.domain;
  if (cookie.path) result.path = cookie.path;
  if (typeof cookie.expires === "number" && Number.isFinite(cookie.expires) && cookie.expires > 0) result.expires = cookie.expires;
  ...
  if (typeof cookie.sourcePort === "number" && Number.isFinite(cookie.sourcePort)) result.sourcePort = cookie.sourcePort;
  return result;
}

/**
 * Mantém os pares nome→valor para compatibilidade e adiciona, em uma chave
...
/**
 * Restaura metadados completos quando disponíveis. Para registros antigos,
 * mantém o comportamento histórico de aplicar nome/valor à URL alvo.
 */
export function recordToPuppeteerCookies(
  ...
  if (jar) {
    return jar.map((cookie) => {
      const restored: PuppeteerSetCookie = { ...cookie };
      // Cookies host-only antigos podem não trazer domínio; nesse caso a URL
      // de destino continua sendo necessária para o Puppeteer.
      if (!restored.domain) restored.url = url;
      return restored;
    });
  }
  return Object.entries(record)
    .filter(([name, value]) => name && name !== COOKIE_JAR_V2_KEY && typeof value === "string")
    .map(([name, value]) => ({ name, value, url }));
}

/** Usado quando uma sessão precisa virar cabeçalho Cookie para HTTP direto. */
export function cookieRecordToHeader(record: Record<string, string> | null | undefined): string {
  if (!record) return "";
  return Object.entries(record)
    .filter(([name, value]) => name && name !== COOKIE_JAR_V2_KEY && typeof value === "string")

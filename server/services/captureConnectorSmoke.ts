...
function readPositiveInteger(name: string): number {
  ...
  if (!raw || !Number.isInteger(value) || value <= 0) {
    throw new Error(`${name} deve conter um ID inteiro positivo.`);
  }
  return value;
}
...
export async function runCaptureConnectorSmoke(): Promise<SmokeResult[]> {
  ...
  const tambasaConfigId = readPositiveInteger("CAPTURE_SMOKE_TAMBASA_CONFIG_ID");
  const searchConfigId = readPositiveInteger("CAPTURE_SMOKE_SEARCH_CONFIG_ID");
  const searchQuery = readRequiredString("CAPTURE_SMOKE_SEARCH_QUERY");

  // O smoke confirma autenticação + extração sem executar uma varredura ampla.
  // Esses limites afetam somente este processo e não alteram configuração no banco.
  ...
  const tambasa = await runSmoke({
    ...
    scraperConfigId: tambasaConfigId,
    mode: "full",
  ...
  const searchOnly = await runSmoke({
    ...
    scraperConfigId: searchConfigId,
    mode: "search",
    query: searchQuery,
  });

...
export async function recordQuotationFeedback(input: {
  ...
  const [item] = await db
    .select({ id: emailQuotationItems.id, quotationId: emailQuotationItems.quotationId })
...
type FeedbackAccumulator = { positive: number; negative: number; selected: number; rejected: number };

async function loadFeedbackSignals() {
  const db = await getDb();
  const byProduct = new Map<number, FeedbackAccumulator>();
  const bySupplier = new Map<number, FeedbackAccumulator>();
  if (!db) return { byProduct, bySupplier };
  ...
  const bump = (map: Map<number, FeedbackAccumulator>, id: number, kind: string) => {
    const current = map.get(id) ?? { positive: 0, negative: 0, selected: 0, rejected: 0 };
    if (["match_approved", "product_selected", "supplier_selected"].includes(kind)) {
      current.positive++;
      current.selected++;
    }
    if (["product_rejected", "supplier_rejected"].includes(kind)) {
      current.negative++;
      current.rejected++;
    }
    map.set(id, current);
  ...
  for (const row of rows) {
    ...
    const productId = numberOrNull(c.productId);
    const supplierId = numberOrNull(c.supplierId);
    const previousProductId = numberOrNull(c.previousProductId);
    const previousSupplierId = numberOrNull(c.previousSupplierId);
    if (productId) bump(byProduct, productId, kind);
    if (supplierId) bump(bySupplier, supplierId, kind);
    if (kind === "product_selected" && previousProductId) bump(byProduct, previousProductId, "product_rejected");
    if (kind === "supplier_selected" && previousSupplierId) bump(bySupplier, previousSupplierId, "supplier_rejected");
  }
  return { byProduct, bySupplier };
}

async function supplierOutcomeStats(supplierIds: number[]) {
  const db = await getDb();
  const result = new Map<number, { uses: number; wins: number; losses: number }>();
  if (!db || supplierIds.length === 0) return result;
  const rows = await db
...
export async function getEnhancedSupplierScore(productId: number) {
  ...
  const feedback = await loadFeedbackSignals();
  const outcomes = await supplierOutcomeStats(ranking.map((r) => r.supplierId));

  return ranking
    .map((row) => {
      const fb = feedback.bySupplier.get(row.supplierId) ?? { positive: 0, negative: 0, selected: 0, rejected: 0 };
      const outcome = outcomes.get(row.supplierId) ?? { uses: 0, wins: 0, losses: 0 };
      const decided = outcome.wins + outcome.losses;
      const winRate = decided > 0 ? (outcome.wins / decided) * 100 : 50;
      const preference = fb.positive + fb.negative > 0
        ? Math.max(0, Math.min(100, 50 + (fb.positive - fb.negative) * 12.5))
        : 50;
      const reliability = Math.max(0, Math.min(100,
        row.scoreBreakdown.availability * 0.45 + row.scoreBreakdown.freshness * 0.20 + winRate * 0.25 + preference * 0.10,
      ));
      const finalScore =
...
export async function getCommercialProtection(quotationId: number) {
  ...
  for (const item of summary.items) {
    ...
    if (item.priceFreshness === "stale" || item.priceFreshness === "unknown") {
    ...
    if (best?.landedCost != null && item.cost != null && item.cost > 0 && best.landedCost < item.cost * 0.95) {
      const saving = item.cost - best.landedCost;
      alerts.push({ severity: "info", code: "cheaper_supplier", itemId: item.itemId, message: `Item ${item.numeroItem ?? item.itemId}: há fonte com economia estimada de R$ ${saving.toFixed(2)} por unidade.` });
    }
  }
  ...
  if (db && data.quotation.orgao) {
    const history = await db
      ...
      .limit(30);
    const values = history.map((r) => numberOrNull(r.valorVencedor)).filter((v): v is number => v != null && v > 0);
  ...
  return {
    ...
    historicalSamples,
    canProceed: alerts.every((a) => a.severity !== "red"),
  };
}
...
export async function refreshStaleQuotationPrices(quotationId: number, userId?: number | null) {
  ...
  if (!db) throw new Error("Banco indisponível.");
  const summary = await getQuotationDecisionSummary(quotationId);
  const stale = summary.items.filter((i) => i.productId && ["stale", "unknown"].includes(i.priceFreshness));
  const supplierIds = new Set<number>();
  for (const item of stale) {
    if (!item.productId) continue;
    const ranking = await rankSuppliersForProduct(item.productId);
    for (const row of ranking.slice(0, 3)) supplierIds.add(row.supplierId);
  }
  if (supplierIds.size === 0) return { staleItems: stale.length, initiated: 0, skippedRecent: 0, suppliers: [] as number[] };
  ...
  const skippedRecent = configs.length - eligible.length;
  for (const cfg of eligible) {
    const job = cfg.scraperType.toLowerCase() === "tambasa"
      ? expandAndSyncTambasaCatalog(cfg.id).then((r) => r.scraper)
  ...
  await recordAudit({
    ...
    changes: { staleItems: stale.length, supplierIds: [...supplierIds], scraperConfigIds: eligible.map((c) => c.id) },
  });

  return { staleItems: stale.length, initiated: eligible.length, skippedRecent, suppliers: [...supplierIds] };
}
...
export async function runQuotationBulkAction(input: {
  ...
  const target = summary.items.filter((item) => selected.size === 0 || selected.has(item.itemId));
  let affected = 0;

  if (input.action === "approve_safe") {
    for (const item of target.filter((i) => i.risk === "green" && i.productId && !i.matchConfirmed)) {
      await db.update(emailQuotationItems).set({ matchConfirmado: true, matchAuto: false }).where(eq(emailQuotationItems.id, item.itemId));
      affected++;
    }
  } else if (input.action === "use_best_supplier") {
    for (const item of target.filter((i) => i.productId)) {
      const ranking = await getEnhancedSupplierScore(item.productId!);
      const best = ranking[0];
      if (!best?.effectivePrice) continue;
  ...
  } else if (input.action === "apply_margin") {
    ...
    for (const item of preview.items.filter((i) => (selected.size === 0 || selected.has(i.quotationItemId)) && i.custoUnitario != null)) {
  ...
  } else if (input.action === "refresh_prices") {
    const refreshed = await refreshStaleQuotationPrices(input.quotationId, input.userId);
    affected = refreshed.initiated;
  }
  ...
  await recordAudit({
    ...
    origin: "operator",
    summary: `Ação em lote ${input.action}: ${affected} item(ns)/fonte(s) afetado(s)`,
...
export async function getCommercialIntelligence() {
  ...
  const salePrices = await getQuotationItemSalePrices(rows.map((r) => r.itemId));
  type Agg = { id: number; name: string; uses: number; wins: number; losses: number; marginSum: number; marginN: number; supplierId?: number | null };
  const productsMap = new Map<number, Agg>();
  const suppliersMap = new Map<number, Agg>();
  const categoriesMap = new Map<number, Agg>();

  const bump = (map: Map<number, Agg>, id: number, name: string, row: typeof rows[number], margin: number | null, supplierId?: number | null) => {
    const agg = map.get(id) ?? { id, name, uses: 0, wins: 0, losses: 0, marginSum: 0, marginN: 0, supplierId };
    agg.uses++;
    if (row.resultado === "ganhou") agg.wins++;
    if (row.resultado === "perdeu") agg.losses++;
    if (margin != null) { agg.marginSum += margin; agg.marginN++; }
    map.set(id, agg);
  };

  for (const row of rows) {
    const cost = numberOrNull(row.cost ?? row.productPrice);
    const sale = numberOrNull(salePrices.get(row.itemId));
    const margin = cost != null && sale != null && sale > 0 ? ((sale - cost) / sale) * 100 : null;
    if (row.productId) bump(productsMap, row.productId, row.productName ?? row.descricao, row, margin, row.supplierId);
    if (row.supplierId) bump(suppliersMap, row.supplierId, row.supplierName ?? `Fornecedor ${row.supplierId}`, row, margin);
    if (row.categoryId) bump(categoriesMap, row.categoryId, row.categoryName ?? `Categoria ${row.categoryId}`, row, margin);
  }

  const format = (agg: Agg) => {
    ...
    return {
      ...
      wins: agg.wins,
      losses: agg.losses,
      winRate: decided > 0 ? Number(((agg.wins / decided) * 100).toFixed(1)) : 0,
      avgMargin: agg.marginN > 0 ? Number((agg.marginSum / agg.marginN).toFixed(1)) : null,
      supplierId: agg.supplierId ?? null,
    };
  };

  const topProducts = [...productsMap.values()].map(format).sort((a, b) => b.wins - a.wins || b.uses - a.uses).slice(0, 12);
  const topSuppliers = [...suppliersMap.values()].map(format).sort((a, b) => b.wins - a.wins || b.winRate - a.winRate || b.uses - a.uses).slice(0, 12);
  const categoryRows = [...categoriesMap.values()].map(format).sort((a, b) => b.wins - a.wins || b.uses - a.uses).slice(0, 12);

  const priorityPriceUpdates = (await Promise.all(topProducts.slice(0, 20).map(async (p) => {
    const source = productsMap.get(p.id);
    const supplierId = source?.supplierId ?? null;
    if (!supplierId) return null;
    const history = await getPriceHistory(p.id, supplierId, 1);
    const freshness = classifyFreshness(history[0]?.recordedAt ?? null);
    return {
      productId: p.id,
      name: p.name,
      uses: p.uses,
      wins: p.wins,
      priority: p.uses + p.wins * 3,
      freshness: freshness.status,
      daysOld: freshness.daysOld,
    };
  }))).filter((x): x is NonNullable<typeof x> => x != null && ["stale", "unknown", "attention"].includes(x.freshness))
    .sort((a, b) => b.priority - a.priority)
    .slice(0, 10);

  const quotations = await db
    .select({ resultado: emailQuotations.resultado, valorProposto: emailQuotations.valorProposto, valorVencedor: emailQuotations.valorVencedor })
    .from(emailQuotations)
    .orderBy(desc(emailQuotations.receivedAt))
    .limit(1000);
  const decided = quotations.filter((q) => q.resultado === "ganhou" || q.resultado === "perdeu");
  const wins = decided.filter((q) => q.resultado === "ganhou").length;
  const gaps = decided.filter((q) => q.resultado === "perdeu").map((q) => {
    const proposed = numberOrNull(q.valorProposto); const winner = numberOrNull(q.valorVencedor);
    return proposed != null && winner != null && winner > 0 ? ((proposed - winner) / winner) * 100 : null;
  }).filter((v): v is number => v != null);
  const allMargins = rows.map((row) => {
    const cost = numberOrNull(row.cost ?? row.productPrice); const sale = numberOrNull(salePrices.get(row.itemId));
  ...
  return {
    overview: {
      ...
      avgMargin: allMargins.length ? Number((allMargins.reduce((a, b) => a + b, 0) / allMargins.length).toFixed(1)) : 0,

...
describe("sessão: assinar → verificar (round-trip)", () => {
  it("aceita a sessão mesmo sem VITE_APP_ID no ambiente", async () => {
    ...
    const token = await sdk.createSessionToken("local:adm@vetmg.com.br", { name: "Administrador" });
    const sessao = await sdk.verifySession(token);
    expect(sessao).not.toBeNull();
    expect(sessao?.openId).toBe("local:adm@vetmg.com.br");
  });

  it("aceita a sessão mesmo quando o usuário não tem nome", async () => {
    const { sdk } = await import("./sdk");
    const token = await sdk.createSessionToken("local:sem-nome@x.com", { name: "" });
    const sessao = await sdk.verifySession(token);
    expect(sessao?.openId).toBe("local:sem-nome@x.com");
  });

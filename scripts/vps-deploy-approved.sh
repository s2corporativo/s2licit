...
# Fecha a janela entre a aprovação e a fase mutável: se main avançar enquanto
# o build ocorre, a imagem recém-construída não é implantada.
git -C "$SOURCE_DIR" fetch --prune origin main
...
# Um segundo fetch após o rsync impede implantação se main avançar exatamente
# durante a cópia. Produção ainda não foi trocada neste ponto.
git -C "$SOURCE_DIR" fetch --prune origin main
LATEST_SHA="$(git -C "$SOURCE_DIR" rev-parse origin/main)"
